...
static void QuakeTextureVecs( plane_t *plane, vec_t shift[ 2 ], vec_t rotate, vec_t scale[ 2 ], qboolean is220, vec_t mappingVecs[ 2 ][ 4 ] ){
	vec3_t vecs[2];
	int sv, tv;
	vec_t ang, sinv, cosv;
	vec_t ns, nt;
	...
	if (is220)
	{
		VectorCopy(mappingVecs[0], vecs[0]);
		VectorCopy(mappingVecs[1], vecs[1]);

		//rotate is included only for purposes of regenerating the surface vectors if the surface is edited. its not needed when those vectors are explicit anyway.
		rotate = 0;
		//scale IS needed though.
	}
	else
...
static void ParseRawBrush( qboolean onlyLights ){
	...
	while ( 1 )
	{
		...
		/*
		    ...
		    by C_DETAIL, defined in vmap.h. the value is exactly as it was before, but
		...
		if ( g_bBrushPrimit == BPRIMIT_OLDBRUSHES ) {
			QuakeTextureVecs( &mapplanes[ planenum ], shift, rotate, scale, is220, side->vecs );
		}
	}
...
/*Spike: we only notice that its a func_detail AFTER we have parsed the entity. So go back and flag the brushes as detail instead.*/
...
//				ParsePatch( onlyLights, qfalse );
	...
	if (!Q_stricmp("misc_prefab", classname)) {
		numEntities--;
//		AddScriptToStack(ValueForKey(mapEnt, "model"), 0);
		return qtrue;
	}

	/* worldspawn (and func_groups) default to cast/recv shadows in worldspawn group */
	if ( funcGroupType!=funcgroup_not || mapEnt->mapEntityNum == 0 ) {
		//%	Sys_Printf( "World:  %d\n", mapEnt->mapEntityNum );
	...
	if (funcGroupType == funcgroup_detail_illusionary) {
		ForceBrushesToDetail( mapEnt, qtrue );
	}
	else if (funcGroupType == funcgroup_detail) {
		ForceBrushesToDetail( mapEnt, qfalse );
	}

	/* group entities are just for editor convenience, toss all brushes into worldspawn */
	if ( !noCollapseGroups && funcGroupType != funcgroup_not ) {
